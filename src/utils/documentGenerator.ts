 ...
 */
export function generateComprehensiveWebsiteContent(title: string, _url?: string): string {
  let cleanTitle = title.replace(/\.[^/.]+$/, '').replace(/[-_]/g, ' ').trim();
  if (cleanTitle.toLowerCase() === 'coarai') {
    cleanTitle = 'CoarAI';
...
${cleanTitle} provides an advanced AI Agent platform engineered to automate customer support, streamline enterprise knowledge management, and execute autonomous workflows with precision and high intelligence.

## Core Capabilities
- **Intelligent Conversational Engine**: Real-time ChatGPT-level responses grounded strictly in company documentation and real-time data sources.
- **Autonomous Tool Calling**: Safely executes API integrations, lead captures, meeting scheduling, and system lookups.
- **Omnichannel Deployment**: Embeddable chat widgets, custom dashboards, webhooks, and RESTful API endpoints.
- **Enterprise Guardrails**: Automated escalation, content moderation, and human-in-the-loop review triggers.

## Architecture & Security
- **Multi-Tenant Isolation**: Robust workspace partitioning, credential protection, and end-to-end audit logging.
- **High-Performance Semantic Retrieval**: Section-aware indexing, vector similarity search, and low-latency answer synthesis.
- **Continuous Reliability**: Resilient failover, business continuity protocols, and uptime monitoring.`;
}
